...
| 子模块 | 类型 | 作用 |
| --- | --- | --- |
| `state` | `DFF<CoreState>` | 保存 CPU 控制状态，包括 `pc` 和 sticky `trap`。 |
| `reg_file` | `RegFileUnit` | 保存 32 个通用寄存器，提供两个读端口和一个写回端口。 |
| `step` | `Rv32iStep` | 根据当前状态、寄存器读数据、指令和数据读值，计算下一状态、寄存器总线、访存请求和提交 trace。 |
| `gpio` | `Gpio` | 捕获写到 `GPIO_BASE` 的 store，并把其他访存请求透传给外部数据存储器。 |

`CoreState::default()` 使 PC 为 `0`、`trap` 为 `false`。寄存器堆和 GPIO 的复位值也是 `0`。
...
2. 从 `q.reg_file.rdata` 取出当前寄存器读口数据，和当前状态、外部 `inst`、`dmem_rdata` 一起送入 `Rv32iStep`。
3. 读取 `q.step` 得到 `StepOutput`，其中包含下一 CPU 状态、寄存器读写总线、取指地址、原始数据访存请求和提交 trace。
4. 把 `step_out.reg_bus` 送入 `RegFileUnit`，在下一个时钟沿提交写回，并用读地址驱动读端口。
5. 把 `step_out.dmem_req` 送入 `Gpio`。
6. 读取 `q.gpio` 得到 GPIO 处理后的输出：GPIO 命中时更新 `gpio_pins` 并吞掉该访存请求，未命中时透传给外部数据存储器。
7. 把 `step_out.state` 写到 `d.state`，等待下一个时钟沿提交。

简化后的路径如下：

```text
q.state + q.reg_file.rdata + inst + dmem_rdata
    -> Rv32iStep
    -> step_out.state      -> d.state
    -> step_out.reg_bus    -> RegFileUnit
...
InstFields -> RegRead -> RegBus.read
RegFileUnit.rdata     -> rs1/rs2 values
...
| `RegRead` | `units/reg_read.rs` | 生成 `rs1`/`rs2` 读地址，并把寄存器堆读数据送给执行单元。 |
| `RegFileUnit` | `units/reg_file.rs` | 由 32 个 `Register32` 元件组成，通过 `RegBus` 接收读地址和写回请求。 |
...
`finish_step` 完成架构状态和寄存器总线提交：

- 只有在 `rd_write == true`、`rd != x0`、没有 illegal、当前未 trap 时才发出寄存器写回请求。
- `RegFileUnit` 忽略对 `x0` 的写入，并保持 `x0 = 0`。
...
- 寄存器堆已从 `CoreState` 拆出为独立 `RegFileUnit`，内部由 32 个 `Register32` 元件组成，还没有映射成 FPGA RAM 宏。
